...
module.exports = {
	createdCategoryModel: (arr, callback) => {
		let sql = `INSERT INTO ${table} (name_category, picture) VALUES ('${arr[0]}', '${arr[1]}')`
		...
		db.query(sql, (err, result, _field) => {
			if (!err) {
				if (result.length) {
					...
					let sql = `SELECT * FROM ${table} WHERE ${arr[0]} LIKE '%${arr[1]}%' LIMIT ${arr[2]} OFFSET ${arr[3]}`
					db.query(sql, (err, result, _field) => {
						if (!err) {
	...
	readCategoryIdModel: (id, callback) => {
		let sql = `SELECT * FROM ${table} WHERE id_category = ${id}`
	...
	readAllCategoryJoinModel: (arr, callback) => {
		...
		let sql = `SELECT  ${table}.id_category, ${table2}.id_item, ${table2}.name, ${table}.name_category FROM ${table2} INNER JOIN ${table} ON ${table2}.id_category = ${table}.id_category WHERE ${table}.id_category = ${arr[2]}`
		// eslint-disable-next-line no-unused-vars
	...
	updatePatchCategoryModel: (arr, callback) => {
		let sql = `SELECT * FROM ${table} WHERE id_category = ${arr[1]}`

		// eslint-disable-next-line no-unused-vars
		db.query(sql, (err, result, _field) => {
			// console.log(result.length)
			if (result.length) {
				let sql = `UPDATE ${table} SET name_category = '${arr[0]}', picture = '${arr[2]}', updated_at = '${arr[3]}' WHERE id_category = ${arr[1]}`
	...
	deleteCategoryModel: (id, callback) => {
		...
		db.query(sql, (err, result, _field) => {
			if (result.length) {
				let sql = `DELETE FROM ${table} WHERE id_category = ${id}`
