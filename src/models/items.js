...
module.exports = {
	...
	createItemModel: (arr, callback) => {
		let sql = `INSERT INTO items (name, price, description, id_category, picture, id_color, id_condition) VALUES ('${arr[0]}', ${arr[1]}, '${arr[2]}', ${arr[3]}, '${arr[4]}', ${arr[5]}, ${arr[6]})`
	...
	updatePutItemModel: (arr, callback) => {
		...
		db.query(sql, (err, result, field) => {
			...
			if (result.affectedRows > 0) {
				let sql = `SELECT * FROM ${table} WHERE id_item = ${arr[3]}`
				// eslint-disable-next-line no-unused-vars
				db.query(sql, (err, result, field) => {
					if (!err) {
						callback(result)
	...
	updatePatchItemModel: (arr, callback) => {
		...
		let sql = `UPDATE items SET ${arr[0]} WHERE id_item = ${arr[1]}`
		// eslint-disable-next-line no-unused-vars
		db.query(sql, (err, result, fields) => {
			if (result.affectedRows > 0) {
				let sql = `SELECT * FROM items WHERE id_item = ${arr[1]}`
				// eslint-disable-next-line no-unused-vars
				db.query(sql, (err, result, fields) => {
					if (!err) {
						callback(result)
	...
	deleteItemModel: (id, callback) => {
		...
		db.query(sql, (err, result, _field) => {
			if (result.length) {
				let sql = `DELETE FROM ${table} WHERE id_item = ${id}`
				// eslint-disable-next-line no-unused-vars
				db.query(sql, (err, result, field) => {
					callback(result)
				})
